...
| Paper candidate lab | Paper-only candidate audit trail | `src/runtime/paperCandidateLab.js`, `src/runtime/candidateExplainability.js`, `src/runtime/decisionSupportDiagnostics.js`, `src/runtime/learningEvidencePipeline.js`, `src/runtime/dashboardPayloadNormalizers.js` | `test/paperCandidateLab.tests.js` | Candidate learning diagnostics | `paperCandidateLabSummary` | Diagnostics only in live | Records generated/blocked/approved/skipped/shadow candidates without execution permission changes. |
...
| Root blocker staleness verifier | Governance/read-only blocker evidence | `src/runtime/rootBlockerStalenessVerifier.js`, `src/runtime/rootBlockerOrchestrator.js`, `src/runtime/dashboardPayloadNormalizers.js` | `test/rootBlockerStalenessVerifier.tests.js` | Root blocker observability | `rootBlockerStalenessSummary` | No unlock/live relief | Marks stale-suspect only for non-hard blockers with fresh evidence; hard safety, unresolved intents and manual review remain dominant. |
| Paper net-edge calibration workbench | Paper-only execution realism analytics | `src/runtime/paperNetEdgeCalibrationWorkbench.js`, `src/runtime/netEdgeGate.js`, `src/execution/microstructureFillSimulator.js`, `src/runtime/dashboardPayloadNormalizers.js` | `test/paperNetEdgeCalibrationWorkbench.tests.js` | Paper fee/slippage calibration diagnostics | `paperNetEdgeCalibrationSummary` | No live threshold relief | Compares realized vs simulated paper drag by symbol/session/order style and recommends calibration only. |
| Shadow strategy tournament | Shadow-only challenger comparison | `src/runtime/shadowStrategyTournament.js`, `src/runtime/featureActivationGovernor.js`, `src/runtime/replayPackScoring.js`, `src/runtime/dashboardPayloadNormalizers.js` | `test/shadowStrategyTournament.tests.js` | Challenger-vs-champion diagnostics | `shadowStrategyTournamentSummary` | No execution/live impact | Persists `shadow_challenger` records and keeps hard safety dominant. |
...
- Root blocker staleness verification is read-only and never clears blockers or unlocks entries.
- Paper net-edge calibration is paper-only diagnostics and cannot lower live thresholds or enable live gates.
- Shadow strategy tournaments simulate alternatives only; they cannot place orders or mutate portfolio state.
