...
import { normalizePaperModeProfileId, resolvePaperModeProfile } from "./paperModeProfile.js";
import { buildConfigProfileAudit } from "./profileAudit.js";
...
export async function loadConfig(projectRoot = process.cwd()) {
  ...
  const config = {
    ...
    envPath,
    botMode,
    exchangeProvider: (env.EXCHANGE_PROVIDER || DEFAULTS.exchangeProvider || "binance").trim().toLowerCase(),
    accountProfile: (env.ACCOUNT_PROFILE || DEFAULTS.accountProfile || "paper").trim().toLowerCase(),
    ...
    postReconcileMaxOpenPositions: parseNumber(env.POST_RECONCILE_MAX_OPEN_POSITIONS, DEFAULTS.postReconcileMaxOpenPositions),
    postReconcilePaperMaxOpenPositions: parseNumber(env.POST_RECONCILE_PAPER_MAX_OPEN_POSITIONS, DEFAULTS.postReconcilePaperMaxOpenPositions),
    postReconcileMaxNewEntriesPerCycle: parseNumber(env.POST_RECONCILE_MAX_NEW_ENTRIES_PER_CYCLE, DEFAULTS.postReconcileMaxNewEntriesPerCycle),
    postReconcileMaxTotalExposureMultiplier: parseNumber(env.POST_RECONCILE_MAX_TOTAL_EXPOSURE_MULTIPLIER, DEFAULTS.postReconcileMaxTotalExposureMultiplier),
    postReconcileLiveSizeMultiplier: parseNumber(env.POST_RECONCILE_LIVE_SIZE_MULTIPLIER, DEFAULTS.postReconcileLiveSizeMultiplier),
    postReconcilePaperSizeMultiplier: parseNumber(env.POST_RECONCILE_PAPER_SIZE_MULTIPLIER, DEFAULTS.postReconcilePaperSizeMultiplier),
  ...
  adaptiveLearningMinQuarantineEvidence: parseNumber(env.ADAPTIVE_LEARNING_MIN_QUARANTINE_EVIDENCE, DEFAULTS.adaptiveLearningMinQuarantineEvidence),
    ...
    enablePullbackReclaimV2: parseBoolean(env.ENABLE_PULLBACK_RECLAIM_V2, DEFAULTS.enablePullbackReclaimV2),
    enableNetEdgeGate: parseBoolean(env.ENABLE_NET_EDGE_GATE, DEFAULTS.enableNetEdgeGate),
    netEdgeGateLiveBlockOnly: parseBoolean(env.NET_EDGE_GATE_LIVE_BLOCK_ONLY, DEFAULTS.netEdgeGateLiveBlockOnly),
    minNetEdgeBps: parseNumber(env.MIN_NET_EDGE_BPS, DEFAULTS.minNetEdgeBps),
    netEdgeSafetyBufferBps: parseNumber(env.NET_EDGE_SAFETY_BUFFER_BPS, DEFAULTS.netEdgeSafetyBufferBps),
    netEdgeExecutionPainBps: parseNumber(env.NET_EDGE_EXECUTION_PAIN_BPS, DEFAULTS.netEdgeExecutionPainBps),
    enableFailedBreakoutDetector: parseBoolean(env.ENABLE_FAILED_BREAKOUT_DETECTOR, DEFAULTS.enableFailedBreakoutDetector),
    failedBreakoutRiskThreshold: parseNumber(env.FAILED_BREAKOUT_RISK_THRESHOLD, DEFAULTS.failedBreakoutRiskThreshold),
    enableFundingOiMatrix: parseBoolean(env.ENABLE_FUNDING_OI_MATRIX, DEFAULTS.enableFundingOiMatrix),
    enableSpotFuturesDivergence: parseBoolean(env.ENABLE_SPOT_FUTURES_DIVERGENCE, DEFAULTS.enableSpotFuturesDivergence),
    enableLeadershipContext: parseBoolean(env.ENABLE_LEADERSHIP_CONTEXT, DEFAULTS.enableLeadershipContext),
    enableSectorRotation: parseBoolean(env.ENABLE_SECTOR_ROTATION, DEFAULTS.enableSectorRotation),
    ...
    liveFastObserveOnly: parseBoolean(env.LIVE_FAST_OBSERVE_ONLY, DEFAULTS.liveFastObserveOnly),
    ...
    releaseChannel: (env.RELEASE_CHANNEL || DEFAULTS.releaseChannel).trim().toLowerCase(),
    canaryTradingEnabled: parseBoolean(env.CANARY_TRADING_ENABLED, DEFAULTS.canaryTradingEnabled),
    canaryMaxTradesPerDay: parseNumber(env.CANARY_MAX_TRADES_PER_DAY, DEFAULTS.canaryMaxTradesPerDay),
    canaryMaxPositionFraction: parseNumber(env.CANARY_MAX_POSITION_FRACTION, DEFAULTS.canaryMaxPositionFraction),
    canaryAllowedSymbols: parseCsv(env.CANARY_ALLOWED_SYMBOLS, DEFAULTS.canaryAllowedSymbols),
    canaryAllowedStrategies: parseTextCsv(env.CANARY_ALLOWED_STRATEGIES, DEFAULTS.canaryAllowedStrategies),
    trainingMode: parseBoolean(env.TRAINING_MODE, DEFAULTS.trainingMode),
    localOnlyMode: parseBoolean(env.LOCAL_ONLY_MODE, DEFAULTS.localOnlyMode),
    ...
    neuralAutonomyEnabled: parseBoolean(env.NEURAL_AUTONOMY_ENABLED, DEFAULTS.neuralAutonomyEnabled),
    neuralAutonomyLevel: parseNumber(env.NEURAL_AUTONOMY_LEVEL, DEFAULTS.neuralAutonomyLevel),
    neuralReplayEngineEnabled: parseBoolean(env.NEURAL_REPLAY_ENGINE_ENABLED, DEFAULTS.neuralReplayEngineEnabled),
    neuralReplayMaxCasesPerRun: parseNumber(env.NEURAL_REPLAY_MAX_CASES_PER_RUN, DEFAULTS.neuralReplayMaxCasesPerRun),
    neuralReplayMinTrades: parseNumber(env.NEURAL_REPLAY_MIN_TRADES, DEFAULTS.neuralReplayMinTrades),
    neuralReplayMinBaselineDelta: parseNumber(env.NEURAL_REPLAY_MIN_BASELINE_DELTA, DEFAULTS.neuralReplayMinBaselineDelta),
    neuralReplayMaxDrawdownPct: parseNumber(env.NEURAL_REPLAY_MAX_DRAWDOWN_PCT, DEFAULTS.neuralReplayMaxDrawdownPct),
    neuralReplayMaxEce: parseNumber(env.NEURAL_REPLAY_MAX_ECE, DEFAULTS.neuralReplayMaxEce),
    neuralContinuousLearningEnabled: parseBoolean(env.NEURAL_CONTINUOUS_LEARNING_ENABLED, DEFAULTS.neuralContinuousLearningEnabled),
    neuralRetrainMinNewPaperTrades: parseNumber(env.NEURAL_RETRAIN_MIN_NEW_PAPER_TRADES, DEFAULTS.neuralRetrainMinNewPaperTrades),
    neuralRetrainMinNewLiveTrades: parseNumber(env.NEURAL_RETRAIN_MIN_NEW_LIVE_TRADES, DEFAULTS.neuralRetrainMinNewLiveTrades),
    neuralRetrainMinNewReplayTrades: parseNumber(env.NEURAL_RETRAIN_MIN_NEW_REPLAY_TRADES, DEFAULTS.neuralRetrainMinNewReplayTrades),
    neuralRetrainMaxPerDay: parseNumber(env.NEURAL_RETRAIN_MAX_PER_DAY, DEFAULTS.neuralRetrainMaxPerDay),
    neuralAutoPromotePaper: parseBoolean(env.NEURAL_AUTO_PROMOTE_PAPER, DEFAULTS.neuralAutoPromotePaper),
    neuralAutoPromoteLive: parseBoolean(env.NEURAL_AUTO_PROMOTE_LIVE, DEFAULTS.neuralAutoPromoteLive),
    neuralSelfTuningEnabled: parseBoolean(env.NEURAL_SELF_TUNING_ENABLED, DEFAULTS.neuralSelfTuningEnabled),
    neuralSelfTuningPaperOnly: parseBoolean(env.NEURAL_SELF_TUNING_PAPER_ONLY, DEFAULTS.neuralSelfTuningPaperOnly),
    neuralSelfTuningMaxActiveExperiments: parseNumber(env.NEURAL_SELF_TUNING_MAX_ACTIVE_EXPERIMENTS, DEFAULTS.neuralSelfTuningMaxActiveExperiments),
    neuralMaxThresholdDelta: parseNumber(env.NEURAL_MAX_THRESHOLD_DELTA, DEFAULTS.neuralMaxThresholdDelta),
    neuralMaxSizeMultiplierDelta: parseNumber(env.NEURAL_MAX_SIZE_MULTIPLIER_DELTA, DEFAULTS.neuralMaxSizeMultiplierDelta),
    neuralLiveAutonomyEnabled: parseBoolean(env.NEURAL_LIVE_AUTONOMY_ENABLED, DEFAULTS.neuralLiveAutonomyEnabled),
    neuralLiveAutonomyAcknowledged: env.NEURAL_LIVE_AUTONOMY_ACKNOWLEDGED || DEFAULTS.neuralLiveAutonomyAcknowledged,
    neuralLiveObserveMinTrades: parseNumber(env.NEURAL_LIVE_OBSERVE_MIN_TRADES, DEFAULTS.neuralLiveObserveMinTrades),
    neuralPaperMinTrades: parseNumber(env.NEURAL_PAPER_MIN_TRADES, DEFAULTS.neuralPaperMinTrades),
    neuralLiveAutonomyMaxTradesPerDay: parseNumber(env.NEURAL_LIVE_AUTONOMY_MAX_TRADES_PER_DAY, DEFAULTS.neuralLiveAutonomyMaxTradesPerDay),
    neuralLiveAutonomyMaxPositionFraction: parseNumber(env.NEURAL_LIVE_AUTONOMY_MAX_POSITION_FRACTION, DEFAULTS.neuralLiveAutonomyMaxPositionFraction),
    neuralLiveAutonomyMaxDailyDrawdown: parseNumber(env.NEURAL_LIVE_AUTONOMY_MAX_DAILY_DRAWDOWN, DEFAULTS.neuralLiveAutonomyMaxDailyDrawdown),
    neuralLiveAutonomyRequireProtection: parseBoolean(env.NEURAL_LIVE_AUTONOMY_REQUIRE_PROTECTION, DEFAULTS.neuralLiveAutonomyRequireProtection),
    neuralLiveAutonomyAutoDisableOnLoss: parseBoolean(env.NEURAL_LIVE_AUTONOMY_AUTO_DISABLE_ON_LOSS, DEFAULTS.neuralLiveAutonomyAutoDisableOnLoss),
    neuralRollbackMaxEce: parseNumber(env.NEURAL_ROLLBACK_MAX_ECE, DEFAULTS.neuralRollbackMaxEce),
    neuralRollbackMaxDrawdownPct: parseNumber(env.NEURAL_ROLLBACK_MAX_DRAWDOWN_PCT, DEFAULTS.neuralRollbackMaxDrawdownPct),
    neuralRollbackLossStreak: parseNumber(env.NEURAL_ROLLBACK_LOSS_STREAK, DEFAULTS.neuralRollbackLossStreak),
    ...
    metricsEnabled: parseBoolean(env.METRICS_ENABLED, DEFAULTS.metricsEnabled || false),
    metricsBindLocalOnly: parseBoolean(env.METRICS_BIND_LOCAL_ONLY, DEFAULTS.metricsBindLocalOnly !== false),
    ...
    paperModeProfile: normalizePaperModeProfileId(env.PAPER_MODE_PROFILE || DEFAULTS.paperModeProfile),
  ...
  parsedConfig.paperModeProfileSummary = resolvePaperModeProfile(parsedConfig);
  const configHash = buildConfigHash(parsedConfig);
  parsedConfig.configHash = configHash.hash;
  parsedConfig.configHashDetail = configHash;
  ...
  parsedConfig.profileAudit = profileAudit;
  return parsedConfig;
}
